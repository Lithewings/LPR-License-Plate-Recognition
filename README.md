...
**亮点**：基于LeNet-5模型，使用批量归一化技术，使用更好的ReLU激活函数替换掉双曲正切函数解决梯度消失与过饱和问题,使用早停和dropout等正则化技术提升模型泛化能力   
**未来的可行改进思路**：数据集分组，字符和数字采用不同模型分别训练识别、数据集正则化、增加倾斜车牌识别、增加模糊车牌识别、进行软件结构重构以易于部署、数据增强/铆钉去除...  
...
- **结果可解释性强**：中间结果会保存到 `located_plate.jpg`、`characters/` 和 `results/`，方便展示定位、分割、预测各环节。
