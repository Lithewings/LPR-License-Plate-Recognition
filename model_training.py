...
def validate_and_count_dataset(dataset, dataset_name, class_names):
    ...
    # 训练集验证集可视化
    os.makedirs("results", exist_ok=True)
    plt.savefig(os.path.join("results", f'{dataset_name}_class_distribution.png'))
    plt.close()
...
def train_model(model, train_ds, val_ds, model_name, class_names, epochs=50):
    ...
    model_path = f'model/{model_name}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.keras'
    ...
    history = model.fit(
        ...
        verbose=2,
        shuffle=False  # 👈 告诉 Keras：数据集内部已处理好，你别多管闲事
    )
...
if __name__ == "__main__":
    ...
    # 训练集：保持打乱（默认就是 True，可以不写）
    train_ds = safe_image_dataset_from_directory(TRAIN_PATH, main_class_names, shuffle=True)

    # 验证集：明确关闭打乱（保持顺序，便于调试和复现）
    val_ds = safe_image_dataset_from_directory(VAL_PATH, main_class_names, shuffle=False)
